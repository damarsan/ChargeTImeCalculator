...
struct ParamsViewAC: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 5) {
                Text("AC Charge").font(.title3).bold()
            ...
            maximumValueLabel: {
                ...
                Text("\(capacity, specifier: "%.2f") Kw/h")
                    .foregroundColor(isEditing ? .green : .blue)
                Text("Select contracted load (Kw)").padding()
            ...
            VStackLayout(spacing: 0) {
                ...
                Text("\(Int(desiredCharge))%").foregroundColor(isEditing ? .green : .blue)
            }
